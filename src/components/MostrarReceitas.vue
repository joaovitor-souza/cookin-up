...
<template>
    <section class="mostrar-receitas">
      ...
      <div v-else class="receitas-nao-encontradas">
        ...
        <img src="../assets/imagens-main/sem-receitas.png" alt="Desenho de um ovo quebrado. A gema tem um rosto com uma expressão triste.">
      </div>
...
<script lang="ts">
import { obterReceitas } from '@/http';
import type IReceita from '@/inrterfaces/IReceitas';
import CardReceitas from './CardReceitas.vue';
import SelecionarIngredientes from './SelecionarIngredientes.vue';

// type Paginas = 'MostrarReceitas' | 'SelecionarIngredientes'

export default {
    data() {
        return {
            receitas: [] as IReceita[],
            // conteudo:'MostrarReceitas' as Paginas
        };
    },
    async created() {
        this.receitas = await obterReceitas();
    },
    components: { CardReceitas, BotaoEditarLista, SelecionarIngredientes },
    emit: ['editarLista']
}
</script>
...
<template>
    <section class="mostrar-receita">
        <h1 class="cabecalho titulo-receitas">Receitas</h1>
        <p class="paragrafo-lg resultados">Resultados encontrados: <span>{{ receitas.length }}</span></p>
        <div class="conteudo" v-if="receitas.length" >
            <p class="paragrafo-lg opcoes">Veja as opções de receitas que encontramos com os ingredientes que você tem por
                aí!</p>
            <ul class="receitas">
        ...
        <div v-else class="conteudo">
            <p class="paragrafo-lg opcoes">Ops, não encontramos resultados para sua combinação. Vamos tentar de novo?</p>
            <img src="../assets/imagens-main/sem-receitas.png" alt="Imagem de Ovo quebrado">
        </div>
